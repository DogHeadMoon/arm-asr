...
public:
    ...
    bool process(vector<int16_t> chunk) {
		...
		Ort::Value h_tensor = Ort::Value::CreateTensor<float>(*ortMemory_,
			h_, size, shape.data(), shape.size());
		Ort::Value c_tensor = Ort::Value::CreateTensor<float>(*ortMemory_,
			c_, size, shape.data(), shape.size());
        ...
        */
        ...
        auto output_tensors = ortSession_->Run(Ort::RunOptions{nullptr}, inputNames_.data(),
				ort_inputs.data(), ort_inputs.size(), outputNames_.data(), outputNames_.size());

        float* pprob = output_tensors[0].GetTensorMutableData<float>();
        float* phn = output_tensors[1].GetTensorMutableData<float>();
        float* pcn = output_tensors[2].GetTensorMutableData<float>();
        memcpy(h_, phn, 128*sizeof(float));
        memcpy(c_, pcn, 128*sizeof(float));

        float probability = pprob[0];
